...
export default async function ApprovalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <>
    ...
    <div className="mt-7 flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 pt-6">{job.status === "Pending Approval" ? <>
      <Link href={`/jobs/${job.id}/edit`} className="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold">Edit before approval</Link>
      <form action={changeJobStatus.bind(null, job.id, "Draft")}><button className="rounded-lg border border-rose-200 bg-white px-4 py-2.5 text-sm font-semibold text-rose-700">Reject</button></form>
      <form action={changeJobStatus.bind(null, job.id, "Published")}><button className="rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white">Approve and publish</button></form>
    </> : <p className="text-sm text-slate-500">This job is <strong>{job.status}</strong> and is not awaiting approval.</p>}</div>
  </>;
}
